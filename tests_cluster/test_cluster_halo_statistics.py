...
def test_halostatistics():
    ...
    for _HS in (HS_tinker, HS_castro):
        print(f"  {type(_HS)}")
        print("    window")
        W, dWdx = _HS.window(k_test, R_test)
        ...
        assert_allclose(_HS.radius_M(M_test), _ref)
        print("    delta_c")
        _ref = [1.67614, 1.679731, 1.681962, 1.683365, 1.684267]
        assert_allclose(_HS.delta_c(z_test), _ref, rtol=5e-7)
        print("    get_Delta_crit")
        _ref = [103.543328, 123.635875, 139.317406, 150.428862, 158.024219]
        assert_allclose(_HS.get_Delta_crit(z_test)[:5], _ref)
        print("    sigma_z_R")
        _ref = [4.175587, 3.550908, 2.366934, 1.386543, 0.673829]
        assert_allclose(_HS.sigma_z_R(z_test, R_test)[0, :5], _ref, rtol=5e-3)
        print("    sigma_z_M")
        _ref = [0.905495, 0.796742, 0.696134, 0.603694, 0.519389]
        assert_allclose(_HS.sigma_z_M(z_test, M_test)[0], _ref, rtol=1e-3)
        print("    nu_z_M")
        _ref = [1.851075, 2.103741, 2.407783, 2.776472, 3.227135]
        assert_allclose(_HS.nu_z_M(z_test, M_test)[0], _ref, rtol=5e-3)
        print("    dlns_dlnR")
        _ref = [-0.649273, -0.684757, -0.722659, -0.76278, -0.805423]
        assert_allclose(_HS.dlns_dlnR(z_test, M_test)[0], _ref, rtol=1e-3)
