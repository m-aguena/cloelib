...
from cloelib.observables.clusters.halo_statistics_models import (
    CastroHaloStatisticsModel,
    TinkerHaloStatisticsModel,
)
...
def test_halostatistics():
    ...
    HS_tinker = HaloStatistics(perturbations, TinkerHaloStatisticsModel, overdensity_type="vir")
    HS_castro = HaloStatistics(perturbations, CastroHaloStatisticsModel, overdensity_type="vir")
    ...
    for HS in (HS_tinker, HS_castro):
        ...
        assert_allclose(HS.sigma_z_M(z_test, M_test)[0], _ref, rtol=1e-3)
        print("    nu_z_M")
