...
class CG:
    ...
    def __init__(self,cosmo_dic,profile='NFW',overdensity_type='vir',overdensity=None,two_hale=None,               offcentering=False,rms_off=0.,f_off=0.,zs_max=2.0,mean_nz=0.4,sigma_nz=0.3,alpha_nz=0.4,mshear=0.0,halo_concentration=0.1,
                 two_halo="None", zed_obs_edges=np.linspace(0.2, 1.8, 9), Lambda_obs_edges=np.array([20., 30., 45., 60., 500. ]), 
        ...
        self.overdensity = overdensity#self.theory['obs_specifications']['CG']['overdensity']
        ...
        self.bias = self.theory['obs_specifications']['CG']['bias']  # Tinker10 or Castro23
        ...
        z_min = 1e-5
        z_max = self.zs_max - 1e-5  # correction needed for avoiding zero values in n_zs_norM computation
        self.z_div = 50
        ...
        self.zed_obs_edges = zed_obs_edges #self.theory['obs_specifications']['CG']['zed_obs_edges']        
        ...
        self.sig_lambda_exponent = self.theory['nuisance_parameters']['sig_lambda_exponent']
        ...
        if alpha_cov_Cxi2 is not None:
            ...
            self.alpha_cov_Cxi2 = np.zeros((self.zed_obs_Cxi2_div,self.Lambda_obs_Cxi2_div))
            self.beta_cov_Cxi2  = np.ones((self.zed_obs_Cxi2_div,self.Lambda_obs_Cxi2_div))
            self.gamma_cov_Cxi2 = np.zeros((self.zed_obs_Cxi2_div,self.Lambda_obs_Cxi2_div)) 
        ...
        self.external_richness_selection_function = self.theory['obs_specifications']['CG']['external_richness_selection_function']
        
        if self.external_richness_selection_function == 'CG_ESF':         
           self.int_Plobltr_Dlob = []
           ltr_grid = 10.**(np.linspace(np.log10(5.),np.log10(100.),100))                   
           ztr_grid = np.linspace(0.1,1.8,18)           
           
           for ltab in range(len(self.Lambda_obs_edges) - 1):
               self.int_Plobltr_Dlob.append(interpolate.interp2d(ztr_grid, ltr_grid, 
                                            CG_ricH_seL_funcT[ltab,:,:], kind='cubic',fill_value=None))            
    ...
    def dV_dzdO(self, z):
        ...
        return self.theory['c'] * self.theory['r_z_func'](z)**2. / self.theory['H_z_func'](z) * self.h**3.
    ...
    def rho_crit_0(self):
        ...
        hubble_value = 100.0/3.085677581491367e+19   # H0/h / unit conversion
    ...
    def rho_crit_z(self, z):
        ...
        hubble_value = self.theory['H_z_func'](z)/3.085677581491367e+19 / self.h  # Hz/h / unit conversion
    ...
    def Omm_z(self, z, nu_cdm):
        ...
        if nu_cdm == 'cb':
           return (self.theory['Omm'] - self.theory['Omnu']) * (1.0+z)**3.0* (self.theory['H0']/self.theory['H_z_func'](z))**2.0
        else:
           return self.theory['Omm'] * (1.0+z)**3.0* (self.theory['H0']/self.theory['H_z_func'](z))**2.0
    ...
    def sigma_z_M(self, z, M):
        ...
        return np.sqrt((1/(2.0*np.pi**2) * simps((k**2.0).reshape(1,1,len(k)) 
                                         * self.Pk_def(z,k,self.neutrino_cdm).reshape(len(z),1,len(k)) 
                                         * (W**2.0).reshape(1,len(R),len(k)), k, axis=-1)))
    ...
    def dlns_dlnR(self, z, M):
        ...
        R = self.radius_M(M) # Mpc/h
        W, dWdx    = self.window(k,R)        
        dsigma2_dR = np.pi**-2 * simps(k.reshape(1,1,len(k))**3 
                                       * self.Pk_def(z,k,self.neutrino_cdm).reshape(len(z),1,len(k)) 
    ...
    def f_sigma_nu(self, z, M):
        ...
        return (A * np.sqrt(2.0*a/(np.pi)) *
                np.exp(-a*nu**2.0/2.0) *
                (1.0 + 1.0/(a*nu**2.0)**p) * (nu*np.sqrt(a))**(q-1.0))*nu
    ...
    def lnlambda(self, z, M):
        ...
        return np.log(self.A_l) + self.B_l*np.log(M/(3.e14)) + \
    ...
    def scatter_lnl(self, z, M):
        ...
        return self.sig_A_l + self.sig_B_l*np.log(M/(3.e14)) + \
    ...
    def P_lnlbd(self, z, M, Lambda):
        ...
        return 1.0/(Lambda*np.sqrt(2.0*np.pi*sigmalnl**2.0)) * np.exp(-(np.log(Lambda)-lnlambda1)**2.0/(2.0*sigmalnl**2.0))
    ...
    def scatter_lbdobs_lbd(self, z, lbd):
        ...
        return (self.sig_lambda_norm +self.sig_lambda_z*z[:,np.newaxis])*lbd**self.sig_lambda_exponent
    ...
    def P_lbdobs_lbd(self, z, lbd, lbd_obs):
        ...
        return 1.0/(np.sqrt(2.0*np.pi*sigma_lbdobslbd**2.0)) * np.exp(-(lbd_obs[np.newaxis,np.newaxis,:]-
                                                                        lbd[np.newaxis,:,np.newaxis])**2.0/(2.0*sigma_lbdobslbd**2.0))
    ...
    def P_zobs_z(self, zed_obs, lbd_obs, z):
        ...
        return 1.0/(np.sqrt(2.0*np.pi*sigmazobsz**2.0)) * np.exp(-(zed_obs[:,np.newaxis]-z)**2.0/(2.0*sigmazobsz**2.0))
    ...
    def sigma_crit(self, z, z_sources):
        ...
        d_a_sources = self.theory['d_z_func'](z_sources)*1.e6
        d_a_l = self.theory['d_z_func'](z)*1.e6
        d_m_l = (1.0+z)*d_a_l
        d_m_sources = (1.0+z_sources)*d_a_sources
        d_h = self.theory['c']/(self.theory['H0']*1.e-6)        
        d_a_lens_source = (1.0/(1.0+z_sources) *
                           (d_m_sources * np.sqrt(1.0 +
                                                  self.theory['Omk'] *
                                                  (d_m_l[:,np.newaxis] **2.0/d_h**2.0)) -
                            d_m_l[:,np.newaxis]  * np.sqrt(1.0 +
                                            self.theory['Omk'] *
                                            (d_m_sources**2.0/d_h**2.0))))                                           
    ...
    def n_zs_norM(self, z):
        ...
        n_zs_norM = 1.0/(skewnorm.cdf(self.zs_max,self.alpha_nz,self.mean_nz,self.sigma_nz) -
    ...
    def n_zs(self, z):
        ...
        for z_ind,zed in enumerate(z) : 
            z_s = np.linspace(zed+1.e-5, self.zs_max, self.z_div+1)
    ...
    def m_sig_crit_m1(self, z, zbin):
        ...
        z_s = np.linspace(z+1.e-5, self.zs_max, self.z_div+1,axis=1)
    ...
    def surface_mass_density_cen(self, R, z, c, M, bias="tinker10", force_no_2h=False):
        ...
        Delta_vir = self.get_Delta(self.overdensity_type, z[:,np.newaxis], 'tot', self.overdensity)
        ...
        if self.profile == "NFW":
            ...
            Sigma = 2.0 * rho_s * Rs * F * 1.e-12
	    
        elif self.profile == "BMO":
            Rt = self.trunc_fact * RDelta
            tau = Rt / Rs
            
            m_bmo = (tau**2.0 / (2.0 * (tau**2.0 + 1.0)**3.0 * (1.0 + c) * (tau**2.0 + c**2.0))
                     * (c * (tau**2.0 + 1.0) * (c * (c + 1.0) - tau**2.0 * (c - 1.0) * (2.0 + 3.0 * c) - 2.0 * tau**4.0)
                     + tau * (c + 1.0) * (tau**2.0 + c**2.0) * (2.0 * (3.0 * tau ** 2.0 - 1.0) * np.arctan(c / tau) + tau* (tau**2.0 - 3.0) *
            ...
            term5 = (np.pi * (4.0 * (tau**2.0 + x**2.0)+ tau**2.0 + 1.0)/ (tau**2.0 + x**2.0)**(3.0 / 2.0))
            term6 = ((tau**2.0 * (tau**4.0 - 1.0) + (tau**2.0 + x**2.0) *
                      (3.0 * tau**4.0 - 6.0 * tau**2.0 - 1.0)) / (tau**3.0 * (tau**2.0 + x**2.0)**(3.0 / 2.0)))
    ...
    def surface_mass_density(self, R, z, c, M, bias="tinker10", force_no_2h=False, force_no_off=False):
        r"""
        ...
        """        
        if force_no_off == False and self.offcentering and self.rms_off >= 1.e-4:
            ...
            ir.Sigma_off(
                R,
                self.r_interp,
                self.surface_mass_density_cen(self.r_interp, z, c, M, bias, force_no_2h=False),
                self.rms_off,
                Sigma_off
            )

            Sigma_cen = self.surface_mass_density_cen(R, z, c, M, bias, force_no_2h=False)
    ...
    def excess_surface_mass_density(self, R, z, c, M, bias="tinker10"):
        ...
        if self.profile == "NFW":	    
            ...
            Sigma_mean = 4.0 * rho_s * Rs * (G / x**2.0) * 1.e-12
            ...
            m_bmo = (
		tau**2.0 / (2.0 * (tau**2.0 + 1.0)**3.0 * (1.0 + c) * (tau**2.0 + c**2.0))
		* (c * (tau**2.0 + 1.0) * (c * (c + 1.0) - tau**2.0 * (c - 1.0) * (2.0 + 3.0 * c) - 2.0 * tau**4.0)
                   + tau * (c + 1.0) * (tau**2.0 + c**2.0) * (2.0 * (3.0 * tau ** 2.0 - 1.0) * np.arctan(c / tau) + tau
			                                      * (tau**2.0 - 3.0) *
                                                              np.log(tau**2.0 * (1.0 + c)**2.0 / (tau**2.0 + c**2.0))
                   )
                )
            ...
            G = np.vectorize(self.G_BMO)(x)
            term3 = (
                (np.pi * (3.0 * tau**2.0 - 1.0) + 2.0 * tau
                 * (tau**2.0 - 3.0) * np.log(tau)
                ) / tau
            )
		...
		-tau**2.0 * (tau**4.0 - 1.0) +
            ...
            M_proj = (
		const * term1
		* (term2 + term3 + (term5 + term6 * L) / term4)
            )
            
            Sigma_mean = M_proj / (np.pi * R**2.0) * 1.e-12
        ...
        if self.offcentering:
            ...
            if self.rms_off >= 1.e-4 and self.f_off >= 1.e-4:
            ...
            elif self.rms_off < 1.e-4 and self.f_off >= 1.0:
    ...
    def surface_mass_density_2h(self, R, z, bias, M=1e14):
        ...
        if isinstance(bias, str):
            if bias == "tinker10":
                Delta = self.get_Delta(self.overdensity_type, z, 'tot', self.overdensity) / self.Omm_z(z, nu_cdm='tot')
            ...
            bias_z = np.ones((z.size,M.size))* bias  # If bias is a scalar float, broadcast it
        ...
        for i, z_val in enumerate(z):  # Loop over redshift values
            ...
            Pk_interp = interpolate.InterpolatedUnivariateSpline(kl_array, self.Pk_def(z_val, kl_array, nu_cdm='tot'))
            ...
            Sigma_z = quad_vec(integrand, kl_min * (1.0 + z_val) * D_A[i], kl_max * (1.0 + z_val) * D_A[i], epsrel=1e-1)[0]
            
            Sigma_z *= 1.e-12 * rho_m * bias_z[i] / (2.0 * np.pi * (1.0 + z_val) ** 3.0 * D_A[i] ** 2.0)
    ...
    def excess_surface_mass_density_2h(self, R, z, bias, M=1e14):
        ...
        for i, z_val in enumerate(z):  # Loop over redshift values
            ...
            # Compute Sigma for each mass M
            DeltaSigma_z = quad_vec(integrand, kl_min * (1.0 + z_val) * D_A[i], kl_max * (1.0 + z_val) * D_A[i], epsrel=1e-1)[0]
            DeltaSigma_z *= 1.e-12 * rho_m * bias_z[i] / (2.0 * np.pi * (1.0 + z_val) ** 3.0 * D_A[i] ** 2.0)
    ...
    def bias_tinker10(self, z, M, Delta):
        ...
        return (1. - A_par*nu**a_par/(nu**a_par + self.delta_c(z)**a_par) + B_par*nu**b_par + C_par*nu**c_par).T 
    ...
    def Kl_coeff(self, L):
        ...
        KL    = np.sqrt(np.pi/(2.*ell+1.))*(eval_legendre(ell-1,np.cos(theta))- 
                                                eval_legendre(ell+1,np.cos(theta)))/(2.*np.pi*(1-np.cos(theta))) 
    ...
    def cov_window(self, zbin, ztab, k, L, KL):
        ...
        self.rint[zbin] = 1/Vz *simps(rvec**2. * np.array([spherical_jn(l, kr, derivative=False) for l in range(L+1)]), rvec, axis=-1).T
        
        return (4*np.pi) * np.sum(self.rint[zbin,:,:] * self.rint[:(zbin+1),:,:] * KL[:]**2, axis=-1)
    ...
    def WF_ra(self,z_array,r_array,k_array):
        ...
        r_z_array    = self.Dv_rs_func(z_array) * r_array   # AP correction (adds a redshift dependence)
        r3_TH_filter = r_z_array**3 * 3.0*(np.sin(k_array*r_z_array)-k_array*r_z_array*np.cos(k_array*r_z_array))/(k_array*r_z_array)**3.0

        W_rad = (r3_TH_filter[:,1:,:] - r3_TH_filter[:,:-1,:])/(r_z_array[:,1:,:]**3 - r_z_array[:,:-1,:]**3)

        V_rad = 4.*np.pi/3. * ((r_z_array[:,1:,0])**3 - (r_z_array[:,:-1,0])**3)
    ...
    def Dv_rs_func(self,z):
        ...
        Dv    = ((1+z)**2 * self.theory['d_z_func'](z)**2 * self.theory['c'] * z/self.theory['H_z_func'](z))**(1/3.) 
        
        # isotropic volume distance at fiducial cosmology (assumed for measuring the 2pcf)
        Dv_fid = ((1+z)**2 * self.theory['fid_d_z_func'](z)**2 * self.theory['c'] * z/self.theory['fid_H_z_func'](z))**(1/3.)  
    ...
    def photoz_rsd_correction(self, z, Lambda):
        ...
        ks           = self.k * (sigma_zob*self.theory['c']/self.theory['H_z_func'](z) * self.h).reshape(len(z),1)
        erf_ks       = erf(ks)
        
        corr0 = np.sqrt(np.pi)/(2*ks)*erf_ks
        corr1 = f_gr/ks**3 * (np.sqrt(np.pi)/2 * erf_ks - ks*np.exp(-ks**2))
        corr2 = f_gr**2/ks**5 * (3*np.sqrt(np.pi)/8 * erf_ks - ks/4*(2*ks**2+3)*np.exp(-ks**2))

        # correct for numerical inaccuracy
        corr1[erf_ks<0.02] = 2/3.
        corr2[erf_ks<0.02] = 1/5.
