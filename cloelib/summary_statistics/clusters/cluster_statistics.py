...
class ClusterStatistics:
    ...
    def N_zbin_Lbin_Rbin(self):
        ...
        if self.CG_like_selection in ["CC", "CC_CWL", "CC_Cxi2", "CC_CWL_Cxi2"]:
            ...
            if self.CG_xi2_cov_selection in ["covCC", "covCC_covCxi2"]:
                ...
                for z_bin in range(len(self.z_obs_edges) - 1):
                    ...
                    z_tab = np.linspace(
                        self.z_obs_edges[z_bin],
                        self.z_obs_edges[z_bin + 1],
                        self.z_tab_sig,
                    )
            ...
            for lambda_bin in range(len(self.Lambda_obs_edges) - 1):
                ...
                Plob_M_z[lambda_bin] = simps(
                    Pl_M_z[:, :, :] * Plob_l_z[:, np.newaxis, :], self.Lambda, axis=-1
                )

                # N(lob,ztr)
                n_lbdobs_z[lambda_bin] = simps(
                    Plob_M_z[lambda_bin] * self.dndm_z, self.Mass, axis=1
                )
        ...
        if self.CG_like_selection in ["CC_Cxi2", "CC_CWL_Cxi2"]:
            ...
            # clustering bins
            z_obs_Cxi2_mid = 0.5 * (
                self.z_obs_Cxi2_edges[1:] + self.z_obs_Cxi2_edges[:-1]
            )
            ...
            W_rad, V_rad = self.clustering.WF_ra(
            ...
            for lambda_bin in range(len(self.Lambda_obs_Cxi2_edges) - 1):
                ...
                for z_bin in range(len(self.z_obs_Cxi2_edges) - 1):
                    ...
                    N_int_lbdobs_z_Cxi2 = simps(
                        dV_dzob * n_lbdobs_z_Cxi2, self.z, axis=0
                    )
