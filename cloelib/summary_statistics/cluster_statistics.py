...
#        self.CG_xi2_cov_selection = 'non_CG_ESF'
        ...
        self.zed_obs_edges = zed_obs_edges
        ...
        self.zed_obs_Cxi2_edges = zed_obs_Cxi2_edges
    ...
    def N_zbin_Lbin_Rbin(self): 
        ...
        if self.CG_like_selection in ['CC','CC_CWL','CC_Cxi2','CC_CWL_Cxi2']:
            ...
            for lambda_bin in range(len(self.Lambda_obs_edges)-1):
                ...
                # P(lob|ltr,ztr)
                Plob_l_z  = simps(self.selectionfunction.P_lbdobs_lbd(self.zed, self.Lambda, l_tab), l_tab, axis=-1) 
#                if external_richness_selection_function == 'CG_ESF':
#                   Plob_l_z  = self.int_Plobltr_Dlob[lambda_bin](self.zed, self.Lambda).T  

                # P(ltrM,ztr)
                Pl_M_z  = self.selectionfunction.P_lnlbd(self.zed, self.Mass, self.Lambda) 
                ...
                for z_bin in range(len(self.zed_obs_edges)-1):
                    ...
                    # P(zob|ztr)                  
                    Pzob_z = simps(self.selectionfunction.P_zobs_z(z_tab, self.Lambda_obs_edges[lambda_bin],self.zed), z_tab, axis=0)       
        ...
        if self.CG_like_selection in ['CC_CWL','CC_CWL_Cxi2'] :
            ...
            for rad_bin in range(len(Rad_obs_mid)):                        
                        excess_surface_mass_density = self.profile.excess_surface_mass_density(np.atleast_1d(Rad_obs[rad_bin]), self.zed, 
...
#                                      
        ...
        if self.CG_like_selection in ['CC_Cxi2','CC_CWL_Cxi2']:
            ...
            for lambda_bin in range(len(self.Lambda_obs_Cxi2_edges)-1):
                ...
                # P(lob|ltr,ztr)
                Plob_l_z_Cxi2 = simps(self.selectionfunction.P_lbdobs_lbd(self.zed, self.Lambda, l_tab), l_tab, axis=-1)         
                ...
                photoz_corr0, photoz_corr1, photoz_corr2 = self.clustering.photoz_rsd_correction(self.zed, Lambda_obs_Cxi2_mid[lambda_bin])
                ...
                for z_bin in range(len(self.zed_obs_Cxi2_edges)-1):
                    ...
                    # P(zob|ztr)                  
                    Pzob_z = simps(self.selectionfunction.P_zobs_z(z_tab, self.Lambda_obs_Cxi2_edges[lambda_bin],self.zed), z_tab, axis=0)       
