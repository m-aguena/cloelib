...
class CC:
    def __init__(self, tracer1: Tracer, tracer2: Tracer):
        ...
        def N_zbin_Lbin_Rbin(self):
            ...
            CG_xi2_cov_selection = self.theory["obs_specifications"]["CG"][
                "CG_xi2_cov_selection"
            ]
            external_richness_selection_function = self.theory["obs_specifications"][
                "CG"
            ...
            if CG_like_selection in ["CC", "CC_CWL", "CC_Cxi2", "CC_CWL_Cxi2"]:
                ...
                Lambda_obs_mid = 0.5 * (
                    self.Lambda_obs_edges[1:] + self.Lambda_obs_edges[:-1]
                )
                ...
                if self.bias == "castro23":
                    bias_z = self.bias_castro23(
                        self.zed, self.Mass
                    )  # only work for virial overdensity
                elif self.bias == "tinker10":
                    Delta_bkg = self.get_Delta(
                        self.overdensity_type, self.zed, "tot", self.overdensity
                    ) / self.Omm_z(self.zed, nu_cdm="tot")
                ...
                if CG_xi2_cov_selection in ["covCC", "covCC_covCxi2"]:
                    ...
                    photoz_corr0 = self.photoz_rsd_correction(z_obs_mid, 0)[
                        0
                    ...
                    for z_bin in range(len(self.zed_obs_edges) - 1):
                        ...
                        z_tab = np.linspace(
                            self.zed_obs_edges[z_bin],
                            self.zed_obs_edges[z_bin + 1],
                            self.z_tab_sig,
                        )
                        ...
                        sab[z_bin, : (z_bin + 1)] = (
                            ...
                            * simps(
                                ...
                                * self.cov_window(z_bin, z_tab, self.k, L, KL),
                                self.k,
                                axis=-1,
                            )
                        )
                ...
                for lambda_bin in range(len(self.Lambda_obs_edges) - 1):
                    ...
                    l_tab = np.geomspace(
                        self.Lambda_obs_edges[lambda_bin],
                        self.Lambda_obs_edges[lambda_bin + 1],
                        self.l_m_tab_sig[lambda_bin],
                    )

                    # P(lob|ltr,ztr)
                    Plob_l_z = simps(
                        self.P_lbdobs_lbd(self.zed, self.Lambda, l_tab), l_tab, axis=-1
                    )
                    if external_richness_selection_function == "CG_ESF":
                        Plob_l_z = self.int_Plobltr_Dlob[lambda_bin](
                            self.zed, self.Lambda
                        ).T
                    ...
                    n_lbdobs_z[lambda_bin] = simps(
                        Plob_M_z[lambda_bin] * dndm_z, self.Mass, axis=1
                    )

                    # N(lob,ztr) * bias(lob,ztr)
                    if CG_xi2_cov_selection in ["covCC", "covCC_covCxi2"]:
                        b_n_lbdobs_z[lambda_bin] = simps(
                            Plob_M_z[lambda_bin] * dndm_z * bias_z, self.Mass, axis=1
                        )
                    ...
                    for z_bin in range(len(self.zed_obs_edges) - 1):
                        ...
                        Pzob_z = simps(
                            self.P_zobs_z(
                                z_tab, self.Lambda_obs_edges[lambda_bin], self.zed
                            ),
                            z_tab,
                            axis=0,
                        )
                        ...
                        N_zbin_Lbin[z_bin, lambda_bin] = simps(
                            n_lbdobs_z[lambda_bin] * dV_dzob[z_bin, lambda_bin],
                            self.zed,
                            axis=0,
                        )
                        ...
                        if CG_xi2_cov_selection in ["covCC", "covCC_covCxi2"]:
                            ...
                            Nb_zbin_Lbin[z_bin, lambda_bin] = simps(
                                b_n_lbdobs_z[lambda_bin] * dV_dzob[z_bin, lambda_bin],
                                self.zed,
                                axis=0,
                            )
                ...
                if CG_xi2_cov_selection in ["covCC", "covCC_covCxi2"]:
                    ...
                    cov_zbin_Lbin = SN + (
                        Nb_zbin_Lbin.reshape(
                            1, self.zed_obs_div, 1, self.Lambda_obs_div
                        )
                        * Nb_zbin_Lbin.reshape(
                            self.zed_obs_div, 1, self.Lambda_obs_div, 1
                        )
    ...
    def __init__(self, tracer1: Tracer, tracer2: Tracer):
        ...
        if CG_like_selection in ["CC_CWL", "CC_CWL_Cxi2"]:
            ...
            for rad_bin in range(len(Rad_obs_mid)):
                excess_surface_mass_density = self.excess_surface_mass_density(
                    Rad_obs[rad_bin], self.zed, self.halo_concentration, self.Mass
                )
                ...
                for lambda_bin in range(len(self.Lambda_obs_edges) - 1):
                    excesssurfacemassdensity = simps(
                        Plob_M_z[lambda_bin]
                        * dndm_z
                        * excess_surface_mass_density[rad_bin],
                        self.Mass,
                        axis=1,
                    )
                    ...
                    for z_bin in range(len(self.zed_obs_edges) - 1):
                        g_zbin_Lbin_Rbin[z_bin, lambda_bin, rad_bin] = (
                            ...
                            * simps(
                                self.m_sig_crit_m1(self.zed, z_bin)
                                * dV_dzob[z_bin, lambda_bin]
...
class Cxi2:
    def __init__(self, tracer1: Tracer, tracer2: Tracer):
        ...
        if CG_like_selection in ["CC_Cxi2", "CC_CWL_Cxi2"]:
            ...
            integ_zbin_lbin = np.zeros(
                ((self.zed_obs_Cxi2_div, self.Lambda_obs_Cxi2_div, len(self.k)))
            )

            # clustering bins
            z_obs_Cxi2_mid = 0.5 * (
                self.zed_obs_Cxi2_edges[1:] + self.zed_obs_Cxi2_edges[:-1]
            )
            Lambda_obs_Cxi2_mid = 0.5 * (
                self.Lambda_obs_Cxi2_edges[1:] + self.Lambda_obs_Cxi2_edges[:-1]
            )
            ...
            for lambda_bin in range(len(self.Lambda_obs_Cxi2_edges) - 1):
                ...
                n_b_lbdobs_z_Cxi2 = simps(
                    Plob_M_z_Cxi2 * dndm_z * bias_z, self.Mass, axis=1
                )
                ...
                photoz_corr0, photoz_corr1, photoz_corr2 = self.photoz_rsd_correction(
                    self.zed, Lambda_obs_Cxi2_mid[lambda_bin]
                )
                pk_halo = pk_IR * (
                ...
                for z_bin in range(len(self.zed_obs_Cxi2_edges) - 1):
                    ...
                    z_tab = np.linspace(
                        self.zed_obs_Cxi2_edges[z_bin],
                        self.zed_obs_Cxi2_edges[z_bin + 1],
                        self.z_tab_sig,
                    )
                    ...
                    Pzob_z = simps(
                        self.P_zobs_z(
                            z_tab, self.Lambda_obs_Cxi2_edges[lambda_bin], self.zed
                        ),
                        z_tab,
                        axis=0,
                    )
                    ...
                    N_int_lbdobs_z_Cxi2 = simps(
                        dV_dzob * n_lbdobs_z_Cxi2, self.zed, axis=0
                    )

                    # power spectrum and shot-noise terms
                    sqrt_Pk_zbin_lbin[z_bin, lambda_bin, :] = (
                        simps(
                            (dV_dzob * n_lbdobs_z_Cxi2)[:, np.newaxis]
                            * np.sqrt(pk_halo),
                            self.zed,
                            axis=0,
                        )
                    ...
                    one_over_n_lambdai_lambdaj[z_bin, lambda_bin, lambda_bin] = (
                        V_zob[z_bin] / N_int_lbdobs_z_Cxi2
                    )
